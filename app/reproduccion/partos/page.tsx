...
import { ArrowLeft, Baby, Loader2, Calendar, CheckCircle2, AlertCircle, Sparkles, Save } from "lucide-react";
...
export default function PantallaPartosPage() {
    ...
    // 👇 ARREGLO TS: Definimos que es un array de cualquier objeto (o podrías crear una interface)
    ...
    const handleRegistrarParto = async () => {
        if (!selectedDiag) return alert("Selecciona una vaca de la lista.");
        if (formParto.tipo_parto !== "Aborto" && !formParto.nombre_cria) {
            return alert("Por favor, ponle un nombre a la cría.");
        }
        ...
        try {
            ...
            const payload = {
                ...
                numero_parto: formParto.numero_parto,
                tipo_parto: formParto.tipo_parto,
                // Estos campos los recibirá el backend para crear el animal
                nombre_animal: formParto.nombre_cria,
                sexo: formParto.sexo_cria
            };
            ...
            if (!res.ok) throw new Error("Error al guardar el parto");

            toast({ title: "¡Parto Exitoso!", description: "Cría registrada y madre en lactancia.", className: "bg-emerald-600 text-white" });
                ...
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="grid gap-3">
                        {vacasGestantes.length === 0 ? (
                            <p className="text-zinc-500 italic">No hay vacas esperando parto.</p>
                        ) : (
                ...
                <div className="lg:col-span-1">
                    <Card className="border-zinc-200 shadow-sm">
                        ...
                        <CardContent className="pt-6 space-y-4">
                            {!selectedDiag ? (
                                <div className="text-center py-6 text-zinc-400 text-sm italic">Selecciona una vaca gestante.</div>
                            ) : (
                                <>
                                    <div className="space-y-2">
                                        <Label>Código de Parto</Label>
                                        <Input value={formParto.numero_parto} onChange={(e) => setFormParto({...formParto, numero_parto: e.target.value})} />
                                    </div>
                                    <div className="space-y-2">
                                        <Label>Tipo de Parto</Label>
                                        <Select value={formParto.tipo_parto} onValueChange={(v) => setFormParto({...formParto, tipo_parto: v})}>
                                            <SelectTrigger><SelectValue /></SelectTrigger>
                                            <SelectContent>
                                                <SelectItem value="Normal">Normal</SelectItem>
                                                <SelectItem value="Distocico">Asistido</SelectItem>
                                    ...
                                    {formParto.tipo_parto !== "Aborto" && (
                                        <div className="p-4 bg-amber-50/50 border border-amber-100 rounded-lg space-y-4 mt-4">
                                            ...
                                            <div className="space-y-2">
                                                <Label>Nombre del Ternero/a</Label>
                                                <Input 
                                                    placeholder="Ej: Lucero" 
                                                    value={formParto.nombre_cria} 
                                                    onChange={(e) => setFormParto({...formParto, nombre_cria: e.target.value})} 
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                <Label>Sexo</Label>
                                                <Select value={formParto.sexo_cria} onValueChange={(v) => setFormParto({...formParto, sexo_cria: v})}>
                                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                                    <SelectContent>
                                    ...
                                    <Button onClick={handleRegistrarParto} disabled={loading} className="w-full bg-zinc-900 hover:bg-zinc-800 text-white mt-4">
