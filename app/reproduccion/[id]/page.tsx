...
const editarMontaSchema = baseEditarMontaSchema.refine(data => {
    if (data.tipo_monta === "Inseminación Artificial" && (!data.codigo_pajilla || data.codigo_pajilla.length < 2)) {
        return false;
    }
    return true;
}, {
...
export default function EditarReproduccionPage() {
    ...
    const validateField = (field: keyof typeof formData, value: any) => {
        try {
            const fieldSchema = baseEditarMontaSchema.shape[field as keyof typeof baseEditarMontaSchema.shape];
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reproduccion/montas/${id}`, {
                ...
                body: JSON.stringify({
                    ...
                    animal_macho_id: valid.tipo_monta === "Monta Natural" && valid.animal_macho_id !== "sin-toro" ? Number(valid.animal_macho_id) : null
                }),
            });

            if (!response.ok) throw new Error("Error al actualizar");

            toast({ title: "¡Actualizado!", description: "Cambios guardados.", className: "bg-emerald-600 text-white" });
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <Select value={formData.animal_hembra_id} onValueChange={(v) => {
                                    setFormData({...formData, animal_hembra_id: v});
                                    validateField('animal_hembra_id', v);
                                }}>
                                    <SelectTrigger className={fieldErrors.animal_hembra_id ? "border-red-500" : ""}><SelectValue placeholder="Selecciona" /></SelectTrigger>
                                    <SelectContent>
                                        {vacas.map((v) => <SelectItem key={v.animal_id} value={v.animal_id.toString()}>{v.arete} - {v.nombre || 'Sin nombre'}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                                {fieldErrors.animal_hembra_id && <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle className="h-3 w-3"/>{fieldErrors.animal_hembra_id}</p>}
                            </div>
                            <div className="space-y-2">
                                <Label>Estado</Label>
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <Select value={formData.tipo_monta} onValueChange={(v: any) => setFormData({...formData, tipo_monta: v})}>
                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                    <SelectContent>
                            ...
                            <div className="space-y-2">
                                <Label>Fecha</Label>
                                <Input type="date" value={formData.fecha_programacion} onChange={(e) => setFormData({...formData, fecha_programacion: e.target.value})} />
                            </div>
                        </div>
                        ...
                        {formData.tipo_monta?.includes("Natural") ? (
                            ...
                            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                                ...
                                <Input className={fieldErrors.codigo_pajilla ? "border-red-500" : ""} value={formData.codigo_pajilla} onChange={(e) => setFormData({...formData, codigo_pajilla: e.target.value})} />
                                {fieldErrors.codigo_pajilla && <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle className="h-3 w-3"/>{fieldErrors.codigo_pajilla}</p>}
                            </div>
                        )}
