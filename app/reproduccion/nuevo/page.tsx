...
const reproduccionSchema = baseReproduccionSchema.refine(
  (data) => {
    if (
      data.tipoServicio === "Inseminación Artificial" &&
...
useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        valid.tipoServicio === "Monta Natural" &&
        valid.toroId &&
        valid.toroId !== "sin-toro"
      ) {
        payload.animal_macho_id = Number(valid.toroId);
      } else if (
        valid.tipoServicio === "Inseminación Artificial" &&
        valid.codigo_pajilla
      ) {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Error al registrar servicio reproductivo",
        );
      }
    ...
    <div className="min-h-screen bg-zinc-50 p-8">
      ...
      <Card className="max-w-3xl mx-auto border-emerald-100 shadow-sm">
        <CardHeader className="bg-emerald-50/50 border-b border-emerald-100 pb-6">
          ...
          <CardDescription>
            Anota una nueva monta o inseminación para llevar el control de
            gestación.
          </CardDescription>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              >
                ...
                >
                  <SelectValue placeholder="Selecciona la vaca a servir" />
                </SelectTrigger>
                <SelectContent>
                  {vacas.map((v) => (
                    <SelectItem
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(
                    v: "Monta Natural" | "Inseminación Artificial",
                  ) =>
                    setFormData({
                      ...formData,
                      tipoServicio: v,
            ...
            {formData.tipoServicio === "Monta Natural" ? (
              <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-100">
                ...
                <Select
                  ...
                  onValueChange={(v) => setFormData({ ...formData, toroId: v })}
                >
                  <SelectTrigger className="bg-white">
                    <SelectValue placeholder="Selecciona el toro (Opcional)" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                ...
                <Input
                  ...
                  className={`bg-white ${fieldErrors.codigo_pajilla ? "border-red-500 focus-visible:ring-red-500" : ""}`}
                  value={formData.codigo_pajilla}
                  onChange={(e) => {
                    setFormData({
                      ...formData,
                      codigo_pajilla: e.target.value,
                    });
